...
def create_teacher(session: Session, data: CreateTeacherRequest) -> User:
    ...
    if is_at_least_17(data.date_of_birth.day, data.date_of_birth.month, data.date_of_birth.year) is False:
    ...
    if(data.password is None):
        password = ( f"{data.date_of_birth.day:02d}" f"{data.date_of_birth.month:02d}" f"{data.date_of_birth.year}")
    else:
